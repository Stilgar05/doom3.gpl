...
public:
	...
	virtual void				Printf( const char *fmt, ... )id_attribute((format(gnu_printf,2,3))) = 0;
	...
	virtual void				DPrintf( const char *fmt, ... ) id_attribute((format(gnu_printf,2,3))) = 0;

								// Prints WARNING %s message and adds the warning message to a queue for printing later on.
	virtual void				Warning( const char *fmt, ... ) id_attribute((format(gnu_printf,2,3))) = 0;

								// Prints WARNING %s message in yellow that only shows up if the "developer" cvar is set.
	virtual void				DWarning( const char *fmt, ...) id_attribute((format(gnu_printf,2,3))) = 0;
	...
	virtual void				Error( const char *fmt, ... ) id_attribute((format(gnu_printf,2,3))) = 0;

								// Fatal errors quit all the way to a system dialog box, which is appropriate for
								// static internal errors or cases where the system may be corrupted.
	virtual void				FatalError( const char *fmt, ... ) id_attribute((format(gnu_printf,2,3))) = 0;
